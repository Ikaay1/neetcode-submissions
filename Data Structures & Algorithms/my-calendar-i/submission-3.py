...
class MyCalendar:
    ...
    def book(self, startTime: int, endTime: int) -> bool:
        ...
        def is_clashes(index):
            start_at_index, end_time_at_index = self.booked_events[index]
            if not((endTime <= start_at_index) or (startTime >= end_time_at_index)):
                return True
        ...
        def find_if_opening():
            ...
            if right >= 0 and is_clashes(right):
                return False
            
            if right+1 < len(self.booked_events) and is_clashes(right+1):
                return False
            
            return True


        is_opening = find_if_opening()
